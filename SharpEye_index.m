...
% opens file number 1 in the directory listing, change to another file in
% the list if desired
fid = fopen([datapath d(1).name]);
    ...
    try
        ...
        switch state
            case SETUP 
                ...
                turning_indexes(f1turning+1) = 1;
                if nextf1turning==firstf1turning && framecount>pulses_per_turning
                    if sum(turning_indexes) == 4096     % full scan 
                        minf1turning = 0;
                        maxf1turning = 4095;
                        minf2turning = 0;
                        maxf2turning = 4095;
                    else                                % not full scan
                        minf1turning = find(turning_indexes,1,'first');
                        maxf1turning = find(turning_indexes,1,'last');
                        minf2turning = minf1turning+squint;
                        maxf2turning = maxf1turning+squint;
                    end
...
%                         fprintf(index_fid,'date: %s - fileposition: %d\n',datestr(f2filednum,'yyyymmddHHMMSS'),ftell(fid)-(double(frameheaderlen)*4)-(double(extrasamples)*4));
                        ...
                        % fast forward in the file by 400 million words (32 bit)
                        fseek(fid,400000000*4,0);
        ...
        fclose(fid);
%         fclose(index_fid);
        save([datapath d(1).name(1:12) '_index.mat'],'index');
        save([datapath d(1).name(1:12) '_info.mat'],'fileheader','minf1turning','maxf1turning','minf2turning','maxf2turning');
        return
    end
end

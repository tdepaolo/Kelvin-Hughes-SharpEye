...
function [scandnum,...
          ...
          squint,...
          r,theta,...
          lonp,latp] = read_SharpEye_sector_stitch(filename,byte_index)
...
%                         plot(t1,'g');
    ...
    % reduce the data to KH recommended stitch points, use pulse 1 up to
    % 1NM, then pulse 2 the rest of the way
    % stitch point (1852m)
    r_stitch = find(r>=2000,1,'first');
    f1time = cat(1,f1time(1:r_stitch-1,:,1:2:end), f1time(r_stitch:end,:,2:2:end));
    f1mag = cat(1,f1mag(1:r_stitch-1,:,1:2:end), f1mag(r_stitch:end,:,2:2:end));
    f1phi= cat(1,f1phi(1:r_stitch-1,:,1:2:end), f1phi(r_stitch:end,:,2:2:end));
    f2time = cat(1,f2time(1:r_stitch-1,:,1:2:end), f2time(r_stitch:end,:,2:2:end));
    f2mag = cat(1,f2mag(1:r_stitch-1,:,1:2:end), f2mag(r_stitch:end,:,2:2:end));
    f2phi = cat(1,f2phi(1:r_stitch-1,:,1:2:end), f2phi(r_stitch:end,:,2:2:end));
